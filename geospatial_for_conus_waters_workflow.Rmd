...
Identify each sample's NHD COMID. This COMID will allow site linkages with all datasets based on the NHD.
...
for(i in 1:nrow(sites)){
  sites$comid[i] <- discover_nhdplus_id(sites[i,])
}
```

Link WBD data to each sample.

```{r}
for(i in 1:nrow(sites)){
  hucs[[i]] <- get_huc12(sites[i,], t_srs=4326)
}

site_hucs <- do.call('rbind',hucs) %>%
...
Select sample flowlines ONLY... get NHD info for each sample location.

```{r}
subset_nhdplus(comids = sites$comid,
...
#join sites to NHD data
sites <- sites %>%
  left_join(site_lines,by='comid')
```

```{r}
rm(site_lines,site_hucs)
gc()
```
...
```{r}
#Read in the NHD
og_nhd <- read_csv('data/nhd_table.csv') #read in the complete NHD (in tabular form)

nhd <- og_nhd %>%
  select("COMID","FDATE","RESOLUTION","GNIS_ID","GNIS_NAME","LENGTHKM","REACHCODE","FLOWDIR","FromNode",   "ToNode","Hydroseq","LevelPathI",   "Pathlength",   "TerminalPa", "DnHydroseq",   "FromMeas",     "ToMeas" )
...
watersheds <- subset_sites %>%
  dplyr::mutate(comid_list = map(site, watersheds))

catchments <- unnest(watersheds,cols=comid_list) %>%
  as_tibble() %>%
...
all_catch <- st_read('data/nhd_catchments.shp') %>% rename(comid = FEATUREID) 

grow_catchments <- merge(all_catch, catchments, by = 'comid', all.x = FALSE) %>% 
  st_write(.,'data/site_catchments.shp', delete_layer = TRUE)
...
3) Then, hilariously, deletes the large gigabytes of data we don't use and only keeps the data that matches our sites's NHD comid's.
...
Fill the `epa_categories` vector with the names of the categortized datasets you are interested in downloading (a list of those data sets can be found [here](https://gaftp.epa.gov/epadatacommons/ORD/NHDPlusLandscapeAttributes/StreamCat/HydroRegions/)):
